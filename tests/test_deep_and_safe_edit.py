    ...
    with ZipFile(path,'r') as zin,ZipFile(tmp,'w',ZIP_DEFLATED) as zout:
        for i in zin.infolist():
            ...
            if i.filename=='xl/worksheets/sheet1.xml':
                txt=d.decode();ext='<extLst><ext uri="{CCE6A557-97BC-4b89-ADB6-D9C93CAAB3DF}" xmlns:x14="http://schemas.microsoft.com/office/spreadsheetml/2009/9/main"><x14:dataValidations count="1"><x14:dataValidation type="list"><x14:formula1><xm:f>$J$2:$J$14</xm:f></x14:formula1><xm:sqref>B2:B3</xm:sqref></x14:dataValidation></x14:dataValidations></ext></extLst>';d=txt.replace('</worksheet>',ext+'</worksheet>').encode()
            zout.writestr(i,d)
...
def test_deep_and_safe(tmp_path):
    ...
    out=tmp_path/'edited.xlsx';r=SafeEditor().edit_cells(p,'Data',[EditOperation('B2',99),EditOperation('A3','Changed')],out,True);assert out.exists();assert r.vba_preserved and r.x14_preserved and r.protected_parts_unchanged;assert set(r.changed_parts)=={'xl/worksheets/sheet1.xml'}
